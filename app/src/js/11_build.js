...
function makeMaterials() {
  ...
  MAT.wallFOH = std({ map: TEX.boardConcrete, roughness: 0.92, envMapIntensity: 0.35 });
  ...
  MAT.ceiling = std({ color: 0x1b1917, roughness: 0.95, envMapIntensity: 0.2, side: THREE.DoubleSide });
  ...
  MAT.red = std({ color: 0x8c1d12, roughness: 0.45, envMapIntensity: 0.5 });
...
function buildCeiling(CB) {
  ...
  if (A1 - A0 > 1.5) {
    ...
    const ps = lerp(P0, P1, 0.52), zs = H - 0.12;
    CB.cylAB(MAT.red, [...pAt(A0 - 0.2, ps), zs], [...pAt(A1, ps), zs], 0.032, 10);
...
function buildSign(B, d) {
  ...
  const hm = new THREE.Mesh(new THREE.PlaneGeometry(hw, hh), basic({ map: halo.tex, transparent: true, blending: THREE.AdditiveBlending, depthWrite: false, color: new THREE.Color(1.6, 1.25, 1.0) }));
...
function buildLights(scene) {
  ...
  if (M.tables.length || M.banquettes.length) {
    for (const f of [0.18, 0.5, 0.82]) { const p = ax === 'x' ? [lerp(D[0], D[2], f), rcy(D)] : [rcx(D), lerp(D[1], D[3], f)]; add(p[0], p[1], H - 0.45, 0xffb46e, 11, 9, 'dining'); }
  }
